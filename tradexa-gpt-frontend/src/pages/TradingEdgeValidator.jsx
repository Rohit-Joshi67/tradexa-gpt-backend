...
export default function TradingEdgeValidator() {
  ...
  const L = 1 - W;
  const AW = inputs.avgWin;
  const AL = inputs.avgLoss;
  const F = inputs.fees;

  const grossExpectancy = (W * AW) - (L * AL);
  const netExpectancy = grossExpectancy - F;
  const profitFactor = (L * AL) === 0 ? 99 : (W * AW) / (L * AL);
  const breakEvenWinRate = (AL + F) / (AW + AL + (F*2)); 
  const edge = AL === 0 ? 0 : (netExpectancy / AL) * 100; 

  const expectedR100 = AL === 0 ? 0 : (netExpectancy / AL) * 100;
  
  const probLoss = (n) => Math.pow(L, n) * 100;
  const expectedLosingStreak = L === 0 || inputs.trades <= 1 ? 0 : Math.log(inputs.trades) / Math.log(1 / L);
  
  const payoffRatio = AL === 0 ? 0 : AW / AL;
  const kelly = payoffRatio === 0 ? 0 : W - (L / payoffRatio);
  const maxSuggestedRisk = kelly > 0 ? kelly * 100 * 0.5 : 0; 

  const generateMonteCarlo = () => {
    const data = [];
    let paths = [inputs.capital, inputs.capital, inputs.capital, inputs.capital, inputs.capital];
    
    for (let i = 0; i <= 100; i++) {
      if (i === 0) {
        data.push({ trade: 0, p1: paths[0], p2: paths[1], p3: paths[2], p4: paths[3], p5: paths[4] });
        continue;
      }
      for (let p = 0; p < 5; p++) {
        const isWin = Math.random() < W;
        paths[p] += isWin ? AW - F : -AL - F;
      }
      data.push({ trade: i, p1: paths[0], p2: paths[1], p3: paths[2], p4: paths[3], p5: paths[4] });
    }
    return data;
  };
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div className="bg-neutral-900/50 backdrop-blur-xl border border-white/10 rounded-3xl p-6 relative overflow-hidden">
                ...
                <span className={"text-4xl font-bold " + (netExpectancy > 0 ? "text-emerald-400" : "text-red-400")}>
                  {netExpectancy > 0 ? '+' : ''}{netExpectancy.toFixed(2)}
                </span>
              ...
              <div className={"space-y-8 " + (!isPro ? "opacity-30 pointer-events-none select-none blur-sm" : "")}>
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="bg-neutral-900/50 border border-white/10 rounded-3xl p-6">
                    ...
                    <ul className="space-y-4">
                      <li className="flex justify-between items-center pb-4 border-b border-white/5">
                        ...
                        <span className="font-bold text-white">{expectedLosingStreak.toFixed(1)} trades</span>
                      </li>
                      <li className="flex justify-between items-center pb-4 border-b border-white/5">
                        <span className="text-neutral-300">Prob of 5 Consecutive Losses</span>
                        <span className="font-bold text-amber-400">{probLoss(5).toFixed(2)}%</span>
                      </li>
                      <li className="flex justify-between items-center pb-4 border-b border-white/5">
                        <span className="text-neutral-300">Prob of 10 Consecutive Losses</span>
                        <span className="font-bold text-red-400">{probLoss(10).toFixed(2)}%</span>
                      </li>
                      <li className="flex justify-between items-center">
                        <span className="text-neutral-300">Max Suggested Risk (Half-Kelly)</span>
                        <span className="font-bold text-indigo-400">{maxSuggestedRisk > 0 ? maxSuggestedRisk.toFixed(2) + '%' : 'N/A'}</span>
                      </li>
                    </ul>
                  </div>
                  ...
                  <div className="bg-neutral-900/50 border border-white/10 rounded-3xl p-6">
                    ...
                    <ul className="space-y-4">
                      <li className="flex justify-between items-center pb-4 border-b border-white/5">
                        <span className="text-neutral-300">Expected Return (100 trades)</span>
                        <span className={"font-bold " + (expectedR100 > 0 ? "text-emerald-400" : "text-red-400")}>
                          {(expectedR100 * AL).toFixed(2)} $
                        </span>
                      </li>
                      <li className="flex justify-between items-center pb-4 border-b border-white/5">
                        <span className="text-neutral-300">Expected R per 100 trades</span>
                        <span className="font-bold text-white">+{expectedR100.toFixed(2)} R</span>
                      </li>
                      <li className="flex justify-between items-center pb-4 border-b border-white/5">
                        <span className="text-neutral-300">Payoff Ratio (Reward/Risk)</span>
                        <span className="font-bold text-white">{payoffRatio.toFixed(2)} : 1</span>
                      </li>
                      <li className="flex justify-between items-center">
                        <span className="text-neutral-300">Mathematical Edge</span>
                        <span className={"font-bold " + (edge > 0 ? "text-emerald-400" : "text-red-400")}>{edge.toFixed(2)}%</span>
                      </li>
                    </ul>
                  </div>
                </div>
                ...
                <div className="bg-neutral-900/50 border border-white/10 rounded-3xl p-6 h-[400px] flex flex-col">
                  <div className="flex justify-between items-center mb-6">
                    <h4 className="text-sm font-bold text-neutral-400 uppercase flex items-center gap-2"><Activity className="w-4 h-4 text-indigo-400"/> Monte Carlo Simulation (100 Trades)</h4>
                    <button onClick={() => setMcData(generateMonteCarlo())} className="text-xs bg-white/5 hover:bg-white/10 border border-white/10 px-3 py-1.5 rounded-lg transition-colors">Run Again</button>
                  </div>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={mcData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
